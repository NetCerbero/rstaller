import os
os.environ['DJANGO_SETTINGS_MODULE'] = 'rstaller.settings'
import django
...
def query_log_for_users():
    ...
    """
    return Colector.objects.values('user_id').distinct()
...
def query_log_data_for_user(userid):
    ...
    WHERE user_id = {}
    """
    return Colector.objects.filter(user_id=userid)



def query_aggregated_log_data_for_user(userid):

    user_data = Colector.objects.filter(user_id = userid).values('user_id',
                                                            'content_id',
...
def save_ratings(ratings, user_id, type):
    ...
    for item_id, rating in ratings.items():
        if rating > 0:
            ...
            print ('{} {}'.format(user_id, str(item_id)))
