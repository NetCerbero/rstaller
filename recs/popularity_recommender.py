...
class PopularityBasedRecs(base_recommender):
    ...
    def predict_score(self, user_id, item_id):
        avg_rating = Rating.objects.filter(~Q(user_id=user_id) &
                                           Q(content_id=item_id)).values('content_id').aggregate(Avg('rating'))
        return avg_rating['rating__avg']

    @staticmethod
    def recommend_items_from_log(num=6):
        items = Colector.objects.values('content_id')
    ...
    def recommend_items(self, user_id, num=6):
        pop_items = Rating.objects.filter(~Q(user_id=user_id)).values('content_id').annotate(Count('user_id'),
                                                                                           Avg('rating'))
